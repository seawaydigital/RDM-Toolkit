...
│       ├── HowItWorks.jsx         # Collapsible per-tool explainer (reads toolExplainers.js)
│       ├── InfoCard.jsx
│       ├── NetworkSilence.jsx     # Live outbound-request counter (HowThisWorks verify section)
│       ├── RelatedTools.jsx
│       ├── ResultPanel.jsx
│       ├── SearchBar.jsx
│       ├── ToolCaveats.jsx        # Always-visible pre-use caveats strip (reads TOOL_CAVEATS)
│       ├── ToolSkeleton.jsx
│       ├── Tooltip.jsx
│       └── WelcomeTour.jsx        # First-visit 3-step onboarding modal
├── data/
│   ├── toolRegistry.js            # Single source of truth for all tools and categories
│   ├── toolExplainers.js          # Per-tool explainer copy + TOOL_CAVEATS pre-use caveats map
...
| `pdfjs-dist` | 5.7.284 | PDF rendering and thumbnail extraction (CVE GHSA-wgrm-67xf-hhpq patched in v4+) |
| `@pdf-lib/fontkit` | 1.1.1 | Font embedding in PDFs |
| `jszip` | 3.10.1 | ZIP archive creation and extraction |
| `dompurify` | 3.4.12 | HTML sanitization (Markdown Preview, FileToMarkdown) — kept current via Dependabot (production XSS sanitizer; always take these bumps promptly) |
...
| `vite-plugin-pwa` | 1.3.0 | PWA / service worker generation (Workbox) |
| `@axe-core/cli` | 4.12.1 | Headless axe-core WCAG scans (`npm run a11y:baseline`) |
| `@axe-core/react` | 4.12.1 | Dev-only a11y violation logging in the browser console (`npm run dev`) |
| `http-server` | 14.1.1 | Static server for the axe baseline scan |
...
npm run a11y:baseline    # axe-core WCAG 2.2 AA scan of 10 representative routes → docs/accessibility/
npm run a11y:contrast    # CSS token contrast audit (--strict exits 1 on any AA failure)
```
...
3. ~~**Branch protection**~~ — **enabled 2026-07-12.** Classic rule on `master`: PRs required, 1 approving review, 4 required status checks (`security`, `lighthouse`, `CodeQL`, `bundle-size`), branches must be up to date before merging, no force pushes/deletions. Consequences for workflow: (a) no direct pushes to master — everything goes through a PR; (b) the owner cannot approve their own PRs, so self-authored PRs merge via the admin "Merge without waiting for requirements" bypass; (c) Dependabot npm PRs fail the `security` check by design until the exact-version allowlist in `scripts/security-audit.mjs` is updated in the same PR — check out the PR branch, patch the allowlist, push. **"Require signed commits" was tried and disabled 2026-07-12** — local commits from the dev machine are unsigned; re-enable only after configuring SSH commit signing (key registered on GitHub as a signing key + `git config gpg.format ssh` / `commit.gpgsign true`).
...
| Date | Change |
|---|---|
| 2026-07-17 | **Pre-use tool caveats + PDF form-field detection** — a tester merged PDFs and lost the signature box they needed for a later Adobe Acrobat signing workflow; the limitation was documented only inside the collapsed HowItWorks panel below the tool. **(1) `TOOL_CAVEATS` map + `getCaveats()`** added to [toolExplainers.js](src/data/toolExplainers.js) — 18 tools' highest-impact caveats (feature loss / workflow-order advice / compliance-critical), plain strings only, max 2 per tool. Promotion criteria documented in the map's header comment; the other 28 tools deliberately get none (noise budget). **(2) `ToolCaveats.jsx`** renders an always-visible amber "Before you use this tool" strip; wired globally in App.jsx between the tool header and `<ToolComponent>` — zero edits to tool files. **(3) `pdfFormDetect.js`** — `pdfHasFormFields(bytes)`: pdfjs Widget-annotation scan (same technique as PdfPageInspector), bails on first hit, never throws, slices bytes internally because pdfjs transfers buffers to its worker. **(4) `FormFieldsNotice.jsx`** shared amber notice, wired into the 9 structural pdf-lib tools (merge, split, reorder, delete, rotate, page-numbers, cover-page, watermark, compress) at file-load time — fires only when the uploaded PDF actually contains form fields/signature boxes; Merge PDFs also badges affected file cards ("Form fields"). Non-blocking, advisory only. **(5) merge-pdfs limitations copy rewritten** — now states plainly that form fields and signature boxes do not survive the merge; sign first, then merge. **(6) CSS** — `.tool-caveats*`, `.form-fields-notice`, `.merge-file-formfields`. **Correction to earlier rows:** explainer coverage in `toolExplainers.js` on this branch is 23 tools (Tier 1+2), not the 38–40 claimed by the 2026-04-17 Tier-4 row — the Tier 4 entries are not present in the file; `TOOL_CAVEATS` is a standalone map partly for this reason. **Verified:** `npm run security:audit` passes (46 tools), production build clean (163 precache entries), dev-server checks — caveats callout renders on merge/split/compress/sign/de-identify and not on word-counter, `pdfHasFormFields` returns true/false correctly against a generated signature-field fixture vs a plain PDF, FormFieldsNotice renders with amber styling and role="status", no horizontal overflow at 375px, zero console errors. Spec: docs/superpowers/specs/2026-07-17-tool-caveats-design.md. |
| 2026-07-12→17 | **Branch protection enabled + full Dependabot backlog drained (20 → 1 open PRs)** — (1) **Branch protection on `master`** (classic rule): PRs required, 1 approving review, required checks `security`/`lighthouse`/`CodeQL`/`bundle-size`, up-to-date branches required. "Require signed commits" was enabled, immediately blocked all locally-authored commits (machine has no signing configured), and was disabled — see Known gaps #3 for the re-enable path. (2) **Dependabot triage**: closed 3 obsolete npm PRs (#55/#56/#61 — already satisfied by the 2026-07-07 `npm audit fix`), closed 3 CodeQL-action PRs (#77/#78/#79) superseded by grouped #67, merged 12: codeql-action 4.36.2→4.37.1 matched set (#67/#86/#87 — **init/analyze must always move together**, one skewed PR was hand-patched to the matching SHA), checkout v7 (#76), setup-node v6.4 (#42), upload-artifact v7 (#43), github-script v9 (#50), deploy-pages v5 + upload-pages-artifact v5 (#44/#46, incl. SLSA-step conflict resolution), attest-build-provenance 4.1.1 (#85), vite-plugin-pwa 1.3.0 (#57), pdfjs-dist 5.7.284 (#48), dompurify 3.4.12 (#84). npm PRs each required an allowlist patch in `security-audit.mjs` pushed to the Dependabot branch. (3) **#88 (Vite 8.1.5 + @vitejs/plugin-react 6) left open deliberately** — the deferred breaking migration; do not casually merge. See [docs/HANDOFF.md](docs/HANDOFF.md) for next-session state. |
| 2026-07-07 | **Security refresh + AODA Phases 0–1 (WCAG 2.2 AA scan clean)** — combined pass. **Security:** dompurify 3.4.1 → 3.4.11 (fixes 8 published advisories incl. IN_PLACE sanitization bypass + ALLOWED_ATTR pollution — this is the production XSS sanitizer); `npm audit fix` cleared 8 transitive dev-chain findings (babel, fast-uri, serialize-javascript, brace-expansion); production tree now audits at **0 vulnerabilities**; remaining 2 findings are the documented dev-server-only esbuild/vite advisories (fix = breaking Vite 8 upgrade, deliberately deferred); `npm audit signatures` verifies all 405 packages; guardrail allowlist updated for dompurify 3.4.11 + 3 new a11y devDeps. **Accessibility (executes docs/superpowers/plans/2026-05-03-aoda-compliance-plan.md Phases 0–1):** new devDeps `@axe-core/cli`/`@axe-core/react`/`http-server`; dev-mode axe console logging; `<html lang="en-CA">`; Lighthouse a11y threshold 0.9 → 0.95; `scripts/axe-baseline.mjs` (`npm run a11y:baseline`) scans 10 routes; `scripts/check-contrast.js` + node --test suite (`npm run a11y:contrast`) audits all text-token × bg-token combos. Baseline scan found 1 critical (unlabeled Password Generator sliders) + serious contrast/target-size/link-distinction issues; **all fixed**: `htmlFor`/`id` on sliders, `.sidebar-sister-rs` #D53A3A→#E25555, `--text-muted` #7C9BBF→#86A6C9 (only failing token pair), red-on-card text → #F87171 (HowThisWorks comparison header, DataClassification confidential tier, `.sc-badge--sensitive`), `.tap-agency-badge--nserc` #60A5FA→#93C5FD, `.homepage-cat-count` → text-secondary, LUFA links underlined, welcome-tour dots 24×24 targets (visual dot via `::before`). New: route announcer live region + per-route `document.title` (hash navigation is now announced to screen readers); `.visually-hidden` utility. Post-fix verification: **axe-core 4.12.1 (wcag2a/2aa/21aa/22aa) reports 0 violations on all 10 representative routes**, single `<h1>` per route confirmed. Docs: `docs/accessibility/{baseline-summary,baseline-after-fixes,contrast-audit}-2026-07-07.md` + `patterns.md` conventions. Note: chromedriver could not launch in the audit environment, so the baseline ran the same axe engine injected into a real Chromium session; `npm run a11y:baseline` works in standard environments. Remaining phases 2–6 (ARIA patterns for SearchBar/tabs, canvas-tool alternatives, accessibility statement page) still outlined in the plan doc. |
