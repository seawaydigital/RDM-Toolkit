...
## AODA / WCAG 2.0 Accessibility Status

Current branch evidence as of 2026-05-05:

- `codex/aoda-wcag20-safe-baseline` added the Lakehead accessibility statement, accessibility-barrier feedback topic, full-route axe scan mode, route inventory, and audit evidence package.
- Full automated AODA scan command: `$env:A11Y_TAGS='wcag2a,wcag2aa'; $env:A11Y_ROUTES='all'; npm run a11y:baseline`.
- Latest evidence files: `docs/accessibility/baseline-2026-05-05.json` and `docs/accessibility/baseline-summary-2026-05-05.md`.
- Latest automated result: 57 routes scanned, 0 parse errors, 0 WCAG 2.0 A/AA axe violations.
- Route inventory: `docs/accessibility/wcag20-route-inventory-2026-05-05.md`.
- Audit report: `docs/accessibility/aoda-wcag20-aa-audit-report-2026-05-05.md`.
- Manual test script/results template: `docs/accessibility/manual-test-script-wcag20-aa.md` and `docs/accessibility/manual-test-results-2026-05-05.md`.

Important claim boundary:

- Safe wording: "RDM Toolkit has a clean automated WCAG 2.0 Level A/AA scan across all current public routes."
- Do **not** claim "the entire site is AODA compliant" or "fully WCAG 2.0 AA conformant" until manual keyboard, screen reader, 200% zoom/reflow, and high-risk pointer workflow testing is complete and recorded.
- High-risk manual workflows: `#reorder-pages`, `#sign-pdf`, `#fillable-pdf-form`, `#pdf-redaction`, and `#image-cropper`.
- Any future accessibility PR must keep the full-route WCAG 2.0 scan clean and update the manual evidence docs when testing is performed.
...
- **Pages:** 10 informational pages (not tools) rendered separately from the tool area.
...
│   │   └── AccessibilityStatement.jsx # AODA/WCAG 2.0 status + barrier reporting
│   └── ui/                        # Shared UI primitives
│       ├── ActionButton.jsx
│       ├── DropZone.jsx
│       ├── EncryptedPDFError.jsx
│       ├── ErrorCard.jsx
│       ├── FeedbackModal.jsx      # In-app feedback dialog (mailto + topic selector + accessibility barrier)
│       ├── InfoCard.jsx
│       ├── RelatedTools.jsx
│       ├── ResultPanel.jsx
│       ├── SearchBar.jsx
...
| `#accessibility` | `AccessibilityStatement.jsx` | AODA/WCAG 2.0 Level AA status page for Lakehead use. States current automated readiness, known pointer-heavy workflow limitations, accessible-format support, and accessibility-barrier reporting. The primary barrier button opens `FeedbackModal` with the "Accessibility barrier" topic preselected; mailto fallback targets `INSTITUTION.rdmEmail`. |
...
| Date | Change |
|---|---|
| 2026-05-05 | **AODA / WCAG 2.0 AA automated-readiness branch (`codex/aoda-wcag20-safe-baseline`)** — added `#accessibility` via `AccessibilityStatement.jsx`, added sidebar link and accessibility-barrier reporting through `FeedbackModal` topic selector, added full-route AODA scan mode to `scripts/axe-baseline.mjs` (`A11Y_TAGS=wcag2a,wcag2aa`, `A11Y_ROUTES=all`), and created the evidence package under `docs/accessibility/`. Latest full automated scan: 57 public routes, 0 parse errors, 0 WCAG 2.0 A/AA axe violations. **Claim boundary:** this is automated readiness, not a final AODA conformance claim; manual keyboard, screen reader, 200% zoom/reflow, and high-risk pointer workflow testing must be completed and recorded before saying the whole site is AODA compliant. |
