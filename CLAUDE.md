...
| Date | Change |
|---|---|
| 2026-04-17 | **"How this tool works" explainers extended to 15 Tier 4 tools** — follow-up to the Tier 1 + Tier 2 rollout. Tier 4 (borderline trust — tools that process user files but aren't in the top-stakes set) adds 15 entries to `src/data/toolExplainers.js`, bringing total coverage to **40 of 62 tools**. New entries: Split PDF, Rotate Pages, Reorder Pages, Add Page Numbers, PDF Watermark, Add Cover Page, PDF Page Inspector, Convert Image Format, Resize Image, Image Cropper, Create ZIP, CSV/JSON Converter, CSV Encoding Fixer, CSV Diff, Encoding Detector. Each entry keeps the same five-section structure (What it does / How it works + optional technical accordion / What stays on your device / What to know before you use it / Check for yourself) and reuses `DEFAULT_QUICK_VERIFY` for the Wi-Fi-off test. Library facts verified against source (e.g. rotate uses `page.setRotation(degrees(...))` metadata-only, create-zip uses JSZip DEFLATE level 6, encoding-detector is BOM + UTF-8-validator + high-byte heuristics). Honest limitations surfaced per tool (rotate is display-angle only — pixels don't rotate; CSV parser is position-based; image crop has no feathering; watermark is removable in a full PDF editor). No UI code changes needed — `App.jsx` already auto-renders `<HowItWorks toolId={currentToolId} />` for any tool with an explainer entry. Remaining ~22 tools (pure-text utilities like word-counter, base64-tool, case-converter, calculators, regex-tester, etc.) intentionally skipped as Tier 3 — no file input means no privacy ambiguity to explain. |
