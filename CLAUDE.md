...
- **Tools:** 58 components, all loaded with `React.lazy()` for code splitting. Each tool is a standalone JSX file.
...
- **Offline:** Workbox service worker (via `vite-plugin-pwa`) pre-caches all 136 static assets on first load. All 58 tools work without internet after first visit.
...
├── tools/                         # 58 tool components grouped by category
│   ├── archives/                  # 3 tools
│   ├── calculators/               # 4 tools
│   ├── developer/                 # 2 tools
│   ├── images/                    # 6 tools
...
### Tool count: 58 tools across 9 categories

#### Primary Categories (always visible in sidebar)

| Category | ID | Tools | Count |
|---|---|---|---|
| PDF Tools | `pdf` | merge-pdfs, split-pdf, compress-pdf, rotate-pages, reorder-pages, add-page-numbers, sign-pdf, password-protect-pdf, remove-pdf-password, extract-images-from-pdf, pdf-watermark, pdf-redaction, pdf-page-delete, pdf-to-images | 14 |
...
| `pdfjs-dist` | ^3.11.174 | PDF rendering and thumbnail extraction |
...
- **DOMPurify** sanitizes all Markdown-rendered HTML before it touches the DOM (Markdown Preview tool)
- **zxcvbn** for realistic password strength (not character-class counting)
- **PDF.js** runs PDF parsing in a sandboxed Web Worker (not main thread)
...
### Known gaps (discussed with IT)
1. **No HTTP security headers** — GitHub Pages does not support custom headers. No CSP, X-Frame-Options, or Referrer-Policy. Mitigation: move to Cloudflare Pages or Netlify.
2. **No ZIP bomb protection** — JSZip has no decompressed-size guard. A size check before extraction should be added.
3. **URL-exposed config** — Storage Calculator saves state to URL hash. Hash doesn't go to server logs, but appears in browser history and shared links.
...
| 2026-04-07 | Added PWA service worker (vite-plugin-pwa) for full offline support — all 136 assets pre-cached |
