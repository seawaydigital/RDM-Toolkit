...
5. **xlsx CVEs** — xlsx@0.18.5 has 2 high-severity CVEs (GHSA-4r6h-8v6p-xvw6, GHSA-5pgg-2g8v-p4x9). Not critical (CVSS <9.0), so CI audit passes. No OSS patch available (SheetJS is now commercial for patched versions). Risk is low since users only process their own files.
...
| Date | Change |
|---|---|
| 2026-04-14 | Added File to Markdown tool (`#to-markdown`) — converts DOCX, PDF, HTML, XLSX, CSV, TXT, MD, RTF, JSON to Markdown for AI consumption; two modes: AI-friendly (linearised) and Preserve formatting; uses Turndown.js + mammoth + pdfjs + SheetJS; known: xlsx@0.18.5 has 2 high-severity CVEs (prototype pollution, ReDoS) — not critical-level, no patch available, threat model is user-uploading-own-file so risk is acceptable |
